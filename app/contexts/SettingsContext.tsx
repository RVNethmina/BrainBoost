...
export const SettingsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const loadAllSettings = async () => {
    try {
      const [savedMusic, savedNotifications, savedTextSize, savedTheme] = await Promise.all([
      ...
      if (savedNotifications !== null) setNotificationsState(JSON.parse(savedNotifications));
  ...
  const setBackgroundMusic = async (value: boolean) => {
    ...
    try {
      await AsyncStorage.setItem(STORAGE_KEYS.BACKGROUND_MUSIC, JSON.stringify(value));
    } catch (error) {
