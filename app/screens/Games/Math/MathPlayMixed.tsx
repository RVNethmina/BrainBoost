// app/src/screens/MathPlayMixed.tsx
...
function makeMixedQuestion(): Question {
  const operationType = randInt(0, 2); // 0: addition, 1: subtraction, 2: multiplication
  let a, b, correct;
  ...
  if (operationType === 0) { // Addition
    ...
    a = randInt(10, 100);
    b = randInt(1, a); // Ensure no negative results
    correct = a - b;
  } else { // Multiplication
    a = randInt(1, 12);
    b = randInt(1, 12);
  ...
  const operation = operationType === 0 ? '+' : operationType === 1 ? '-' : '×';
...
const MathPlayMixed: React.FC = () => {
  ...
  const speakQuestion = (q?: Question) => {
    ...
    const operationWord = current.operation === '+' ? 'plus' : 
                         current.operation === '-' ? 'minus' : 
                         'multiplied by';
    const text = `What is ${current.a} ${operationWord} ${current.b}?`;
    Speech.stop();
    ...
    <View className="flex-1 bg-white">
      ...
      >
        ...
        <View className="items-center">
          <Text style={{ fontSize: 22, fontWeight: "700", color: PALETTE.red }}>
            Math Practice
          </Text>
          <Text style={{ fontSize: 16, color: PALETTE.red, marginTop: 2 }}>
            Mixed Operations
          </Text>
        </View>
        <View style={{ width: 40 }} />
          ...
          <View style={{ marginTop: 24 }}>
            ...
            <View style={{ flexDirection: "row", marginTop: 18, gap: 12 }}>
              <TouchableOpacity
                ...
                style={[
                  styles.voiceButton,
                  { backgroundColor: isRunning ? PALETTE.red : "#D1D5DB" }
                ]}
              >
