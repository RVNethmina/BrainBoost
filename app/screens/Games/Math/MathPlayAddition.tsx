// MathPlayAddition.tsx with TTS/STT, Subtraction, matching MathPlayMultiplication style
...
function makeQuestion(minOperand = 1, maxOperand = 50): Question {
  const operation = Math.random() < 0.5 ? 'addition' : 'subtraction';
  let a = randInt(minOperand, maxOperand);
  let b = randInt(minOperand, maxOperand);
  ...
  return { a, b, correctAnswer: correct, options: optionsArr, operation };
}
...
const MathPlayAddition: React.FC = () => {
  ...
  const speakQuestion = (q?: Question) => {
    ...
    if (!current) return;
    const opWord = current.operation === 'addition' ? 'plus' : 'minus';
    const text = `What is ${current.a} ${opWord} ${current.b}?`;
    Speech.stop();
  ...
  const getOperatorSymbol = (op: 'addition' | 'subtraction') => op === 'addition' ? '+' : '−';
    ...
    <View className="flex-1 bg-white">
      ...
      >
        ...
        <View className="items-center">
          <Text style={{ fontSize: 22, fontWeight: "700", color: PALETTE.teal }}>
            Math Practice
          </Text>
          <Text style={{ fontSize: 16, color: PALETTE.teal, marginTop: 2 }}>
      ...
      <View className="flex-1 px-5 py-6">
        <View style={styles.questionCard}>
          <Text style={styles.questionText}>
            {current ? `${current.a} ${getOperatorSymbol(current.operation)} ${current.b} = ?` : "Loading..."}
          </Text>
          ...
          <View style={{ marginTop: 24 }}>
            ...
            <View style={{ flexDirection: "row", marginTop: 18, gap: 12 }}>
              <TouchableOpacity
                ...
                style={[
                  styles.voiceButton,
                  { backgroundColor: isRunning ? PALETTE.teal : "#D1D5DB" }
                ]}
              >
