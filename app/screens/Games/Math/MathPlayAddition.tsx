// app/src/screens/MathPlayAddition.tsx
...
function makeAdditionQuestion(minOperand = 1, maxOperand = 50): Question {
  ...
  while (options.size < 4) {
    // offsets biased small so distractors are plausible
...
const MathPlayAddition: React.FC = () => {
  ...
  const [isRunning, setIsRunning] = useState<boolean>(false); // true when started and not paused
  ...
  const handleAnswer = (selected: number) => {
    if (!isRunning) return; // ignore when not running / paused
  ...
  const endQuiz = (reason: "time" | "finished") => {
    ...
    // navigate to results - cast as any to avoid strict overload issues
