// app/src/screens/AttentionPlayHard.tsx
...
const TARGET_DISPLAY_TIME = 1000; // ms - fastest
const TARGET_SIZE = 50; // smallest
...
function generateTargets(gameWidth: number, gameHeight: number): Target[] {
  ...
  const numTargets = randInt(3, 6); // Even more targets
  const hasRealTarget = Math.random() < 0.35; // 35% chance - harder to find
  ...
  for (let i = 0; i < numTargets; i++) {
    ...
    do {
      ...
      attempts < 8 &&
      targets.some(t => 
        Math.sqrt(Math.pow(t.x - x, 2) + Math.pow(t.y - y, 2)) < TARGET_SIZE + 5
      )
    );
...
const AttentionPlayHard: React.FC = () => {
  ...
  useEffect(() => {
    if (isRunning) {
      ...
      spawnTargets();
      targetIntervalRef.current = setInterval(spawnTargets, TARGET_DISPLAY_TIME + 200);
    } else {
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      {/* Instructions */}
      <View className="px-5 py-4" style={{ backgroundColor: '#FFE0E0' }}>
        <Text className="text-lg font-semibold text-center" style={{ color: PALETTE.red }}>
          HARD: Find RED CIRCLES fast! 🔴
        </Text>
        <Text className="text-sm text-center text-gray-600">
          Many distractors, very fast pace
        </Text>
      </View>
      ...
      >
        ...
        {!isRunning && showStartHint && (
          <View className="items-center justify-center">
            ...
            <Text className="text-2xl font-bold mb-2" style={{ color: PALETTE.red }}>
              Hard Challenge!
            </Text>
            <Text className="text-lg text-center text-gray-600 mb-6 px-4">
              Maximum difficulty! Small, fast targets with many distractors. Focus!
            </Text>
          </View>
        )}
        ...
        {!isRunning && !showStartHint && currentTargets.length === 0 && (
          <View className="items-center justify-center">
            ...
            <Text className="text-xl font-semibold" style={{ color: PALETTE.red }}>
              Paused
            </Text>
          </View>
        )}
